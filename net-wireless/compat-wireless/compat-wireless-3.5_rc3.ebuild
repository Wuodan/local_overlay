...
#	fixme: there are more bluethooth settings in the config.mk
	...
	epatch "${FILESDIR}"/TEST_RM.patch

	# replace calls to /usr/src/linux/Makefile
	# actually the call is to a symlink and it gets "kernelversion"
	# there's one last instance of "make ... kernelversion" in gen-stable-release.sh but should not matter
	# find ./ -type f | xargs sed -i "s/shell \$(MAKE) -C \$(KLIB_BUILD) kernelversion/echo ${KV_FULL}/g" || die "sed failed"
	# find ./ -type f | xargs sed -i "s/\${MAKE} -C \${KLIB_BUILD} kernelversion/echo ${KV_FULL}/g" || die "sed failed"
	# find ./ -type f | xargs sed -i "s/make -C \$KLIB_BUILD kernelversion/echo ${KV_FULL}/g" || die "sed failed"
	for file in compat/scripts/gen-compat-config.sh config.mk defconfigs/atheros-debug.mk scripts/gen-compat-autoconf.sh; do
		sed -i "s/shell \$(MAKE) -C \$(KLIB_BUILD) kernelversion/echo ${KV_FULL}/g" "${file}" || die "sed failed"
		sed -i "s/\${MAKE} -C \${KLIB_BUILD} kernelversion/echo ${KV_FULL}/g" "${file}" || die "sed failed"
		sed -i "s/make -C \$KLIB_BUILD kernelversion/echo ${KV_FULL}/g" "${file}" || die "sed failed"
	done
}
...
src_compile() {
	...
	# die "TEST"
	# addwrite "${DESTDIR}"/usr/src/linux
	# addwrite "${DESTDIR}"/lib/modules/"${KV_FULL}"/build
	# die "TEST"
